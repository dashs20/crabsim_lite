...
def run_sim(plant_config_dict=None, log_filename='log.csv', generate_pdf=True):
    # load guidance table from CSV
    guidance_data = np.loadtxt('guidance.csv', delimiter=',', skiprows=1)
    t_lookup_s = guidance_data[:,0]
    wx_cmd_lookup_radps = lookup_1D(t_lookup_s, deg2rad(guidance_data[:,1]))
    wy_cmd_lookup_radps = lookup_1D(t_lookup_s, deg2rad(guidance_data[:,2]))
    wz_cmd_lookup_radps = lookup_1D(t_lookup_s, deg2rad(guidance_data[:,3]))
    ...
    # initialize GNC previous command to match initial motor speeds
    crabbrain.prev_u_nav[2] = np.abs(crabcopter.px_motor_model.x)
    crabbrain.prev_u_nav[3] = np.abs(crabcopter.nx_motor_model.x)

    # define gyro sensor
    gyro = ICM42688(1000,200,dt_s)
    ...
    gnc_log_array = np.zeros((n_steps,25))
    gnc_log_col_names = ["wx_cmd_radps","wy_cmd_radps","wz_cmd_radps",
                         ...
                         "thr_cmd_frac",
                         "phi_px_est_rad","phi_nx_est_rad","w_px_est_radps","w_nx_est_radps",
                         "phi_px_filt_rad","phi_nx_filt_rad","w_px_filt_radps","w_nx_filt_radps",
                         "wx_filt_radps","wy_filt_radps","wz_filt_radps",
    ...
    for i_step in range(n_steps):
        ...
        gnc_log_array[i_step,:] = np.hstack((
            w_des_radps, 
            w_meas_radps.flatten(), 
            act_cmd, 
            thr_frac_cmd,
            act_est, # raw noisy actuator estimates
            crabbrain.nav.act_est_filtered, # filtered actuator estimates
            crabbrain.nav.w_est_filtered.flatten(), # filtered body rates
            crabbrain.integrator.i_err # integrator accumulated error
        ))
    ...
    np.savetxt(log_filename, log_array, delimiter=',', fmt='%.3f', header=log_col_names, comments='')
    
    if generate_pdf:
        generate_smart_pdf_plots(log_filename)
